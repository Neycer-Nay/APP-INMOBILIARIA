...
@section('contenido')
    ...
    <div class="mb-8 flex justify-between items-center">
        <div>
            <h2 class="text-2xl md:text-3xl font-bold text-[#404656] menu-font">Mis Transacciones</h2>
            <p class="text-gray-500 mt-1 text-sm menu-font">Casas vendidas y registro de ventas</p>
        </div>
        <button onclick="openModal()" class="bg-[#e09129] hover:bg-[#d18a1f] text-white font-semibold py-2 px-4 rounded-lg transition-colors duration-200 flex items-center gap-2">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-4 py-3 text-sm text-green-600 font-medium">
                                @if(isset($venta->detalles) && $venta->detalles->count() > 0)
                                    ...
                                    <span class="text-xs text-gray-500">({{ $venta->detalles->first()->porcentaje_comision }}%)</span>
                                @else
                                    -
                                @endif
                            </td>
                            <td class="px-4 py-3">
                                <button onclick="viewVenta({{ $venta->id }})" class="text-[#404656] hover:text-[#e09129] transition-colors">
    ...
    <!-- Modal Nueva Venta -->
    <div id="ventaModal" class="fixed inset-0 bg-transpaernt bg-opacity-50 hidden z-50 flex items-center justify-center">
            ...
            <form id="ventaForm" action="{{ route('transacciones.store') }}" method="POST" class="p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="casa_id" id="casaSelect" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#e09129]">
                            <option value="">Seleccionar casa...</option>
                            @foreach(\App\Models\Casa::where('agente_id', Auth::user()->agente->id)->whereDoesntHave('venta')->with('propietario')->get() as $casa)
                                <option value="{{ $casa->id }}" data-propietario="{{ $casa->propietario->nombre ?? '' }} {{ $casa->propietario->apellido ?? '' }}" data-precio="{{ $casa->precio }}">
                    ...
                    <div>
                        ...
                        <input type="text" id="propietarioDisplay" readonly class="w-full bg-gray-100 border border-gray-300 rounded-lg px-3 py-2 text-gray-500">
                    </div>

                    <!-- Fecha de Venta -->
                    <div>
                        <label class="block text-sm font-medium text-[#404656] mb-1">Fecha de Venta *</label>
                        <input type="date" name="fecha_venta" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#e09129]">
                    </div>

<!-- Precio de Venta -->
                    <div>
                        <label class="block text-sm font-medium text-[#404656] mb-1">Precio de Venta *</label>
                        <input type="number" name="precio_venta" id="precioVenta" step="0.01" required class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#e09129]">
                    </div>

                    <!-- Cliente (opcional) -->
                    <div>
                        <label class="block text-sm font-medium text-[#404656] mb-1">Cliente (Comprador)</label>
                        <select name="cliente_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#e09129]">
                    ...
                    <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                        <div class="flex justify-between items-center">
                            <span class="text-sm font-medium text-green-700">Comisión del Agente ({{ Auth::user()->agente->comision_predeterminada ?? 10 }}%)</span>
                ...
                <div class="mt-6 flex justify-end gap-3">
                    <button type="button" onclick="closeModal()" class="px-4 py-2 border border-gray-300 rounded-lg text-[#404656] hover:bg-gray-50 transition-colors">
                        Cancelar
                    </button>
                    <button type="submit" class="px-4 py-2 bg-[#e09129] hover:bg-[#d18a1f] text-white rounded-lg transition-colors">
                        Registrar Venta
                    </button>
                </div>
            </form>
